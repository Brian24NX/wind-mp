# 写在最前面：强烈建议先阅读官方教程[Dockerfile最佳实践]
...
# 将构建产物jar包拷贝到运行时目录中
COPY --from=build /app/wind-mp-offiaccount/target/wind-mp-offiaccount.jar .

# 暴露端口
EXPOSE 80

# 执行启动命令
CMD ["java", "-jar", "/app/wind-mp-offiaccount.jar" , "--spring.profiles.active=dev"]
